...
void Device::compile_command_queue_programs() {
    ...
    uint32_t prefetch_d_buffer_pages = PREFETCH_D_BUFFER_SIZE >> PREFETCH_D_BUFFER_LOG_PAGE_SIZE;
            ...
            for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
                ...
                if (device_id == this->id()) {
                    std::map<string, string> dispatch_defines = {
                        ...
                        {"UPSTREAM_NOC_X", std::to_string(prefetcher_physical_core.x)},
                        {"UPSTREAM_NOC_Y", std::to_string(prefetcher_physical_core.y)},
