...
void kernel_main() {
    ...
    uint32_t num_tiles_per_shard = addr_gen.input_shard_num_tiles_x * addr_gen.input_shard_num_tiles_y;
    for (uint32_t t = 0; t < num_transfers; ++t) {

        for (uint32_t i = 0; i < num_pages_per_shard; i += pages_per_eth_l1_buffer) {
            noc_semaphore_wait(writer_send_semaphore_addr_ptr, 1);
            noc_semaphore_set(writer_send_semaphore_addr_ptr, 0);

            while(get_semaphore(sem_addr) <= sem_idx);

            uint32_t num_tiles_to_send = std::min(pages_per_eth_l1_buffer, num_pages_per_shard - i);
            send_pages_to_eth_from_local_output_tensor_shard(cb_id_in0, num_tiles_to_send, page_size, eth_l1_sender_base_noc_addr, eth_l1_sender_semaphore_addr);
